...
def HCTcv(img_path, Dmin, Dmax, dist2R=0.4, param1=200, param2=15,\
             display_img=True,
             export_img=True, export_csv=True, eximgname = None,
             export_edges=True, edges_eximgname=None\
             ):
    """
    ...
    circles = cv2.HoughCircles(work_image,\
    ...
    if display_img==True or export_img==True:
        ...
        if export_edges==True:
            imgE = cv2.Canny(work_image, threshold1=param1, threshold2=0.5*param1)
        else:
            imgE = None
        if display_img==True:
    ...
    if export_csv == True:
        circlesDF.rename(columns={"X":"X (pixels)", "Y":"Y (pixels)", "R":"Radius (pixels)"}, inplace=True)
        circlesDF.index.name = "ID"
        circlesDF.to_csv(img_dir + os.sep + os.path.splitext(img_filename)[0] + "_HCT.csv")
    return circlesDF


def HCTcv_varR(img_path, Dspace, dist2R=0.4, param1=200, param2=15,
                  export_img=True, export_csv=True):
    assert isinstance(Dspace, np.ndarray), "Dspace must be numpy array,"\
    ...
    for i in range(len(Dspace)-1):
        ...
        imname= str(i+1) + "_" + img_filename.split(".")[0] + "_range("\
            + str(int(Dspace[i])) + "-" + str(int(Dspace[i+1])) + ").png"
        print(imname)
        buffDF = HCTcv(img_path, Dspace[i], Dspace[i+1], dist2R,
                          param1, param2,
                          display_img=False,export_csv=False,
                          export_img=export_img, eximgname=imname)
...
def HCTcv_multi(img_path, Dspace, dist2R=0.4,
                param1=200, param2=15,
                export_img=True, export_csv=True, export_edges=True):
    ...
    for i, (D, d2R, p1, p2) in enumerate(zip(Dspace, dist2R, param1, param2)):
        if i < len(Dspace)-1:
            imname= str(i+1) + "_" + img_filename.split(".")[0] + "_D("\
                + str(int(Dspace[i])) + "-" + str(int(Dspace[i+1])) + ")_p1("\
                + str(int(p1)) + ")_p2(" + str(int(p2)) + ")"
            cirname = imname + "_circles.png"
            edgename = imname + "_edges.png"
            buffDF = HCTcv(img_path, Dspace[i], Dspace[i+1], d2R,
                            param1=p1, param2=p2,
                            display_img=False, export_csv=False,
                            export_img=export_img, eximgname=cirname,
